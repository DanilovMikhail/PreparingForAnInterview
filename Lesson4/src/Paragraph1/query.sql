...
From
    (select p.datetime, f.name, f.duration, adddate(p.datetime, INTERVAL (f.duration + 30) MINUTE) date_end from poster p
        inner JOIN film f
        on p.film_id = f.id) as tb1
    inner JOIN (select p.datetime, f.name, f.duration from poster p
                inner JOIN film f
...
From (Select p.id, p.datetime, f.name, f.duration, adddate(p.datetime, INTERVAL f.duration MINUTE) Film_end from poster p
        inner JOIN film f
        on p.film_id = f.id) as tb1
    inner JOIN (Select p.id, p.datetime, f.name, f.duration from poster p
                inner JOIN film f
                on p.film_id = f.id) as tb2
